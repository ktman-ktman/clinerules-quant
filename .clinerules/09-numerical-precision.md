...
- 分散・標準偏差は自前実装せず numpy / polars(またはpandas)の実装を使う(数値的に安定なアルゴリズムが使われている)
...
if not np.isfinite(weights.to_numpy()).all():
    raise ValueError(f"ウェイトに非有限値: null={weights.null_count()}件")
```

- polars の集計関数はデフォルトで null をスキップする。`drop_nulls()` や `fill_null()` で null を「無視してよいか」を都度判断し、意図を明示する
- pandas補完時も同様に、集計関数がデフォルトで NaN をスキップする(`skipna=True`)点に注意する
